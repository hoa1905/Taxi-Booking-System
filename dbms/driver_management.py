...
def driver_select_all_booking(did):
    ...
    sql = """
        SELECT booking.bookingid, booking.pickupaddress, booking.date,
            booking.time, booking.dropoffaddress, customers.name, booking.bookingstatus
        FROM booking
        INNER JOIN customers ON booking.cid = customers.cid
        WHERE booking.did = %s AND booking.bookingstatus = 'Booked'
    """
    values = (did,)
    driver_result = None
...
def driver_trip_history(did):
    ...
    sql = """
        SELECT customers.cid, customers.name, booking.date,
            booking.time, booking.pickupaddress, booking.dropoffaddress
        FROM booking
        INNER JOIN customers ON booking.cid = customers.cid
        WHERE booking.did = %s AND booking.bookingstatus = 'Billing Completed'
    """
    values = (did,)
    driver_result = None
