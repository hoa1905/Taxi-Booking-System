    ...
    def __init__(self, main):
        ...
        def assign_driver():
            ...
            title11_lbl = customtkinter.CTkLabel(
                master=assign_booking_frame, text="Chuyến đi hoàn thành", font=font720
            )
            title11_lbl.place(x=110, y=20)

            bookingid_lbl = customtkinter.CTkLabel(
                assign_booking_frame, text="Mã đặt xe: ", font=font720
            )
            ...
            name_lbl = customtkinter.CTkLabel(
                assign_booking_frame, text="Tên khách hàng:", font=font720
            )
            ...
            status_lbl = customtkinter.CTkLabel(
                assign_booking_frame, text="Trạng thái đặt xe:", font=font720
            )
            status_lbl.place(x=30, y=250)

            combo_data = ("Completed", "Incomplete")
            booking_combo = customtkinter.CTkComboBox(
                assign_booking_frame, values=combo_data, font=font720, width=200
            )
            ...
            assign_booking_frame2 = customtkinter.CTkFrame(master=root, width=840)
            assign_booking_frame2.pack(side=LEFT, fill=BOTH, padx=(0, 10), pady=10)

            booking_table = ttk.Treeview(assign_booking_frame2)
            ...
            booking_table.column("date", width=100, anchor=CENTER)
            booking_table.column("time", width=100, anchor=CENTER)
            ...
            booking_table.heading("name", text="Tên khách hàng", anchor=CENTER)
            booking_table.heading(
                "bookingstatus", text="Trạng thái đặt xe", anchor=CENTER
            )
            ...
            booking_table.pack(padx=10, pady=10)
            ...
            def get_driver_detail(a):
                ...
                name_txt.insert(0, booking_table.item(select_item2)["values"][5])
            ...
            def update_customer_booking():
                ...
                update_booking = BookingLibs(
                    bookingstatus="Bill Pending", bookingid=booking_id_txt.get()
                )
                update_booking_result = driver_update_booking(update_booking)

                driver = DriverLibs(did=driver_id.get(), driverstatus="Active")
        ...
        def switch():
            ...
            if is_on:
                ...
                driver_info = DriverLibs(did=driver_idd, driverstatus="Inactive")
                ...
                is_on = True
                driver_idd = driver_id.get()
                driver_info = DriverLibs(did=driver_idd, driverstatus="Active")
                update_result = update_driver_status(driver_info)
                if update_result == True:
                    toggle_label.configure(
        ...
        def switch2():
            ...
            if select_result != None:
                if select_result[6] == "Active":
                    toggle_button.config(image=on)
                    toggle_label.configure(
                        text="{} đang hoạt động".format(Global.current_driver[1])
                    )
                elif select_result[6] == "Inactive":
                    toggle_button.config(image=off)
                    toggle_label.configure(
                        text="{} không hoạt động".format(Global.current_driver[1])
                    )

                elif select_result[6] == "Booked":
                    toggle_button.config(image=off)
                    toggle_label.configure(
        ...
        scrollbar_y = ttk.Scrollbar(
            center_frame, orient="vertical", command=treeView.yview
        )
        scrollbar_y.place(x=1460, y=250, height=545)  # ngay bên phải Treeview

        # Scrollbar ngang
        scrollbar_x = ttk.Scrollbar(
            center_frame, orient="horizontal", command=treeView.xview
        )
        scrollbar_x.place(x=10, y=800, width=1450)  # ngay bên dưới
        ...
        treeView.heading("bookingstatus", text="Trạng thái đặt xe", anchor=CENTER)
